...
#region COLOR PROPERTIES
    ...
    /// <summary>
    /// The microphone device name to use for MicrophoneInput / StereoMix source types.
    /// Set this before calling RebuildSpectrum() to switch devices at runtime.
    /// Null or empty string selects the system default microphone.
    /// </summary>
    public string overrideMicrophoneName = null;
    ...
    public void RestartMicrophone()
    {
        ...
        if (sourceType == SourceType.MicrophoneInput || sourceType == SourceType.StereoMix)
        {
            ...
            // Allow external override (e.g. from debug UI / PlayerPrefs).
            microphoneName = !string.IsNullOrEmpty(overrideMicrophoneName) ? overrideMicrophoneName : null;

            if (sourceType == SourceType.StereoMix) //find stereo mix
            {
                if (microphoneName == null) // only auto-search when no override is set
                {
